...
router.get('/', async (req, res) => {
  try {
    const result = await db.query('SELECT * FROM baskets');
...
router.get('/:id', async (req, res) => {
  ...
  try {
    const basket = await db.query('SELECT * FROM baskets WHERE id = $1', [id]);
    ...
    const products = await db.query('SELECT * FROM basket_products WHERE basket_id = $1', [id]);
...
router.post('/', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'INSERT INTO baskets (name, description, price, weight, image_url) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [name, description, price, weight, image_url]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'UPDATE baskets SET name = $1, description = $2, price = $3, weight = $4, image_url = $5 WHERE id = $6 RETURNING *',
      [name, description, price, weight, image_url, id]
    );
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await db.query('DELETE FROM basket_products WHERE basket_id = $1', [id]);

    // Supprimer le panier
    const result = await db.query('DELETE FROM baskets WHERE id = $1 RETURNING *', [id]);
...
router.post('/:basketId/products', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'INSERT INTO basket_products (basket_id, product_id, quantity) VALUES ($1, $2, $3) RETURNING *',
...
router.get('/:basketId/products', async (req, res) => {
  ...
  try {
    const result = await db.query('SELECT * FROM basket_products WHERE basket_id = $1', [basketId]);
    res.json(result.rows);
  } catch (error) {
...
router.put('/:basketId/products/:productId', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'UPDATE basket_products SET quantity = $1 WHERE basket_id = $2 AND product_id = $3 RETURNING *',
...
router.delete('/:basketId/products/:productId', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'DELETE FROM basket_products WHERE basket_id = $1 AND product_id = $2 RETURNING *',
      [basketId, productId]
    );
