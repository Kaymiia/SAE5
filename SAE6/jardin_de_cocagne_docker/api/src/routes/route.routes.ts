...
router.get('/routes', async (req, res) => {
  try {
    const result = await db.query('SELECT * FROM delivery_routes');
...
router.get('/routes/:id', async (req, res) => {
  ...
  try {
    ...
    const points = await db.query('SELECT * FROM delivery_points WHERE route_id = $1 ORDER BY sequence_order', [id]);
...
router.post('/', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'INSERT INTO routes (name, start_point, end_point, distance, estimated_time, is_active) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [name, start_point, end_point, distance, estimated_time, is_active ?? true]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'UPDATE routes SET name = $1, start_point = $2, end_point = $3, distance = $4, estimated_time = $5, is_active = $6 WHERE id = $7 RETURNING *',
      [name, start_point, end_point, distance, estimated_time, is_active, id]
    );
