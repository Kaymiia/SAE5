...
router.post('/', async (req, res) => {
    try {
      ...
      const userCheck = await db.query('SELECT * FROM users WHERE id = $1', [userIdNum]);
      if (userCheck.rows.length === 0) {
        return res.status(404).json({ message: 'Utilisateur non trouvé' });
      }
  
      // Vérifier l'existence du panier
      const basketCheck = await db.query('SELECT * FROM baskets WHERE id = $1', [basketIdNum]);
      if (basketCheck.rows.length === 0) {
        return res.status(404).json({ message: 'Panier non trouvé' });
      }
  
      // Vérifier l'existence du point de livraison
      const deliveryPointCheck = await db.query('SELECT * FROM delivery_points WHERE id = $1', [deliveryPointIdNum]);
      ...
      const existingSubscription = await db.query(
        'SELECT * FROM user_subscriptions WHERE user_id = $1 AND basket_id = $2',
        [userIdNum, basketIdNum]
      );
      ...
      // Créer l'abonnement
      const result = await db.query(
        'INSERT INTO user_subscriptions (user_id, basket_id, delivery_point_id, status) VALUES ($1, $2, $3, $4) RETURNING *',
        [userIdNum, basketIdNum, deliveryPointIdNum, status || 'active']
      );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const subscription = await db.query('SELECT * FROM user_subscriptions WHERE id = $1', [id]);
    ...
    const result = await db.query(
      'UPDATE user_subscriptions SET user_id = $1, basket_id = $2, delivery_point_id = $3, status = $4, updated_at = NOW() WHERE id = $5 RETURNING *',
      [user_id, basket_id, delivery_point_id, status, id]
    );
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await db.query('DELETE FROM user_subscriptions WHERE id = $1', [id]);
